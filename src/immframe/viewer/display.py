...
from . import mat_image

# Video is handled by the controller via python-mpv (see immframe.video.player),
# NOT through this viewer. The dead-code stubs below keep the original picframe
# slideshow path image-only without invasive surgery.
...
class VideoStreamer:
    ...
    def play(self, path): return False
    def kill(self): pass


class VideoFrameExtractor:
    def __init__(self, *a, **kw): pass
    def get_first_and_last_frames(self): return None
...
class ViewerDisplay:
    ...
    def __orientate_image(self, im, pic):
        # Immich's preview JPEGs are already rotated server-side; no further
        # transposition needed. Method kept for call-site compatibility.
        return im
